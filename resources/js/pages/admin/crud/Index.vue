    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4">
            <div class="mb-5 flex items-center justify-between">
                ...
                <Link :href="route(resource.routeBase + '.create')" class="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        <tr v-for="(row, i) in rows" :key="row.id" class="hover:bg-slate-50 dark:hover:bg-slate-900/30">
                            ...
                            <td class="px-4 py-3 text-right">
                                <Link :href="route(resource.routeBase + '.edit', row.id)" class="text-sm font-medium text-brand-700 hover:underline">Edit</Link>
                                <button @click="destroy(row.id)" class="ml-3 text-sm font-medium text-rose-600 hover:underline">Delete</button>
                            </td>
                        </tr>
